...
registerUser(user: any) {
  const url_api = "http://200.10.147.68:3000/auth/register";

  return this.http.post(url_api, user);
}

loginuser(email: string, password: string): Observable<any> {
  const url_api = "http://200.10.147.68:3000/auth/login";
  return this.http.post(url_api, {email, password});
}

passwordUser(password: any) {
  const url_api = "http://200.10.147.68:3000/auth/changePass";
